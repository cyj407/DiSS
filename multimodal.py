...
def main():
    ...
    os.makedirs(os.path.join(os.getcwd(), args.save_name), exist_ok=True)

    sketch = load_sketch(
        data_dir=args.input_sketch_dir,
    ...
    stroke_input = Image.open(args.input_stroke_img).convert("RGB")
    stroke_input = stroke_input.resize( (args.image_size, args.image_size), Image.LANCZOS)
    stroke_input = (
        TF.to_tensor(stroke_input).to(dist_util.dev()).unsqueeze(0).mul(2).sub(1)
    )

    for _type in ['cat', 'dog', 'wildlife']:
        ...
        for num in range(args.num_samples):
            ...
            def model_fn(x_t, sketch, stroke, ts, sketch_guidance_scale=1.0, stroke_guidance_scale=1.0):
            ...
            save_npz_img(
                name=os.path.join(os.getcwd(), args.save_name, '{}{}.jpg'.format(_type, num)), 
...
def create_argparser():
    defaults = dict(
        encode_step=250,
        clip_denoised=True,
        input_sketch_dir = './test-examples/multimodal',
        input_stroke_img = './test-examples/stroke-example.jpg',
        save_name = 'multimodal-output',
        num_samples=5,
        batch_size=1,
        use_ddim=False,
        model_path = "./checkpoints/afhq.pt",
