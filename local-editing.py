...
def main():
    ...
    import cv2
    from img_process import extract_black_contour, extract_color_strokes, grab_cut, extract_sketch_and_strokes
    ...
    for num in range(args.num_samples):    
        ...
        for sketch_guidance_scale in [1.5]:
...
def create_argparser():
    defaults = dict(
        encode_step=250,
        clip_denoised=True,
        input_image = './test-examples/local-example.png',
        save_name = 'local-output',
        num_samples=20,
        batch_size=1,
        use_ddim=False,
